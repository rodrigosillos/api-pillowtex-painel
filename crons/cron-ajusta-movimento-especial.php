...
$sql = "select m.cod_operacao from movimentacao m where m.notas is null and m.representante_cod = '0054' and m.data_emissao between '2022-04-01' and '2022-04-31'";
...
foreach ($movimentacoes as $movimentacaoSemNF) {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $sql = "select comissao_r, tabela from movimentacao where notas like '%" . $numeroNota[0] . "%'";
        $stmt = $pdo->prepare($sql);
