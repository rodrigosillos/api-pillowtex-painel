...
foreach ($resultListaMovimentacao['value'] as $valueListaMovimentacao) {
    ...
    //$sql = "SELECT id FROM invoices WHERE operation_code = :operation_code";
    //$stmt = $pdo->prepare($sql);
    //$stmt->bindParam(':operation_code', $valueListaMovimentacao['cod_operacao'], PDO::PARAM_STR);
    //$stmt->execute();
    ...
    //if ($stmt->rowCount() == 0) {
        ...
        if(isset($resultConsultaMovimentacao['odata.count'])) {
            ...
            print('erro no metodo (movimentacao/consulta): operacao: ' . $valueListaMovimentacao['cod_operacao'] . "\xA");
            $txt = $resultConsultaMovimentacao;
            $myfile = file_put_contents('log-dataload-invoices.txt', $txt.PHP_EOL , FILE_APPEND | LOCK_EX);
            
        }
    
        // if($invoiceAgent == 219 || $invoiceAgent == 117 || $invoiceAgent == 217) {
        //if($invoiceAgent == 148 || $invoiceAgent == 123 || $invoiceAgent == 170) {
        //}        
